...
class WorldModelCache
{
	...
	private static final long MIN_REFRESH_MILLIS = 350L;
			...
			Map<String, Object> identity)
	{
		...
		Snapshot snapshot = refreshIfNeeded(client, options, tick, clientTick, identity);
	...
	private Snapshot refreshIfNeeded(
			Client client,
	...
	private void captureScene(Client client, Snapshot snapshot, QueryOptions options)
	{
		...
		for (int sceneX = 0; sceneX < planeTiles.length; sceneX++)
		{
			...
			for (int sceneY = 0; sceneY < column.length; sceneY++)
			{
				...
				addTileObjects(client, snapshot, tile);
				addGroundItems(client, snapshot, tile, options.maxGroundItems);
			}
		}
		applyObjectProjections(client, snapshot, options);
		snapshot.objectCensusCapHit = snapshot.objects.size() >= HARD_MAX_OBJECTS;
	}
			...
			Tile tile)
	{
		...
		if (wall != null)
		{
			addObject(client, snapshot, "WALL_OBJECT", wall, wall.getOrientationA());
		}
		GroundObject ground = tile.getGroundObject();
		if (ground != null)
		{
			addObject(client, snapshot, "GROUND_OBJECT", ground, -1);
		}
		DecorativeObject decorative = tile.getDecorativeObject();
		if (decorative != null)
		{
			addObject(client, snapshot, "DECORATIVE_OBJECT", decorative, -1);
		}
		...
		for (GameObject gameObject : gameObjects)
		{
			if (gameObject != null)
			{
				addObject(client, snapshot, "GAME_OBJECT", gameObject, gameObject.getOrientation());
			}
		}
	}
			...
			int orientation)
	{
		...
		Map<String, Object> record = objectRecord(client, snapshot, kind, object, orientation);
	...
	private void applyObjectProjections(Client client, Snapshot snapshot, QueryOptions options)
	{
		...
		for (Map<String, Object> record : candidates)
		{
			...
			if (project && projectionBudget.tryConsume())
			{
				...
				record.put("projection", object == null ? projectionUnavailable("object_ref_missing") : projectionPayload(client, object, snapshot));
			}
			else
			{
				record.put("projection", projectionUnavailable(project && projectionBudget.capHit ? "projection_cap_hit" : "not_requested"));
			}
		}
	...
	private Map<String, Object> objectRecord(Client client, Snapshot snapshot, String kind, TileObject object, int orientation)
	{
		Map<String, Object> record = new LinkedHashMap<>();
		int id = object.getId();
		ObjectComposition definition = objectDefinition(client, id);
		String name = definitionName(definition);
		List<String> actions = definitionActions(definition);
	...
	private Map<String, Object> objectCensusPayload(Snapshot snapshot, QueryOptions options)
	{
		...
		filtered.sort(Comparator
				.comparingInt((Map<String, Object> object) -> intValue(object.get("distanceToPlayer"), 9999))
				.thenComparing(object -> stringValue(object.get("name"))));
	...
	private Map<String, Object> fullDebugPayload(Snapshot snapshot, QueryOptions options)
	{
		...
		payload.put("metadata", metadataPayload(snapshot));
		payload.put("scene", scenePayload(snapshot));
		payload.put("objects", compactObjects(snapshot.objects, options.maxObjects));
	...
	private boolean matchesQueryFilters(Map<String, Object> object, QueryOptions options)
	{
		...
		if (options.center != null)
		{
			...
			if (worldX == Integer.MIN_VALUE || worldY == Integer.MIN_VALUE || plane != options.center.plane)
			{
				return false;
			}
			if (Math.max(Math.abs(worldX - options.center.worldX), Math.abs(worldY - options.center.worldY)) > options.radiusTiles)
			{
				return false;
			}
		}
