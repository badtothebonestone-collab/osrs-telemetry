...
| --- | --- | --- | --- |
| 1 | recover_wrong_floor_route_context | Recover the live loop from `3206,3229,1` without loosening route-object matching. | Real live loop obtains a valid route/resource context from the wrong-floor state, sends only expected route actions, and reaches woodcutting area or a precise route-context blocker. |
