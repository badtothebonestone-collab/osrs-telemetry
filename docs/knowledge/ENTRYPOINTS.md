...
| Bot eval | `bot_eval_runner.py` | New live-loop launchers | Replay, preflight, explicit live smoke/dry-run, and guarded live action belong here. Real action requires `--live --execute-actions`; `--live` alone fails closed. |
...
| Routes | `route_template.py`; `route_monitor.py`; `route_demonstration.py` (`resolve_reentry`); `traversal_lifecycle.py` | Bot eval route parsers, raw-click template logic | Use route segments/templates/guides; raw clicks are support evidence. Wrong/intermediate-floor recovery must use route-guide re-entry evidence instead of generic fallback clicks. |
