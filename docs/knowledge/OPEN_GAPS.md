...
| live_route_wrong_floor_recovery | high | route/context/action_proposal | open | Top-floor `Bottom-floor / Staircase` is now proven from live trace and modeled as a strict floor-selection route action. Plane-1 recovery at `3206,3229,1` remains unproven; keep `route_guide_no_same_plane_reentry` if already stranded there, and only use the focused top-floor probe before a full rerun. |
