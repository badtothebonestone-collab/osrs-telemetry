...
| authenticated_start_game_missing | high | startup/recovery | open | Configure or select an authenticated Start Game command; current canonical recovery exhausts Click here, Play Now, and dev Gradle relaunch but still ends at LOGIN_SCREEN with `dev_launch_not_loaded`. |
...
| live_route_wrong_floor_recovery | high | route/context/action_proposal | open | Top-floor `Bottom-floor / Staircase` is proven from live trace and modeled as a strict floor-selection route action. Fresh probe `20260609_185122_plane1_staircase_recovery_probe` also captured strict plane-1 `Climb-down / Staircase` for object `16672` at `3204,3229,1`; next live run should verify the plane-change postcondition without loosening guards. |
