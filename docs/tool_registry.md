...
| `context_service.py` | Query-first current context, blocker explanations, navigation decision trace summaries, and task-script readiness snapshots. No-session named Knowledge Fabric queries use the live daemon first; `--session` and `--latest-session` stay file-session scoped unless `--daemon-url` is explicitly supplied. | `python telemetry-viewer\context_service.py --query current-debug-context`; `python telemetry-viewer\context_service.py --query navigation-decision-trace`; `python telemetry-viewer\context_service.py --query task-script-run-readiness`; MCP: `query_navigation_decision_trace` |
| `task_script_api.py` | High-level task script primitive contract, validation, compile, evidence planning, runtime evidence snapshots/comparison, failure classification, step/run readiness, explanation, and templates. | MCP: `get_task_script_api_spec`, `validate_task_script`, `compile_task_script`, `get_task_script_evidence_plan`, `get_task_script_runtime_evidence`, `compare_task_script_runtime_evidence`, `classify_task_failure`, `assess_task_script_step`, `assess_task_script_run`; direct: `--query task-script-run-readiness` |
