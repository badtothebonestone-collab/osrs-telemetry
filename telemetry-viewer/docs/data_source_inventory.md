...
| task_script_runtime_evidence | read-only runtime evidence | Knowledge Fabric | lifecycle proof comparison | `task_runtime_evidence.v1` | source evidence freshness | evidence variable catalog; `--query task-script-runtime-evidence` | no | no | no |
...
| task_step_readiness | read-only script step gate | Knowledge Fabric and `task_script_api.py` | Codex before bounded script/operator requests | `task_step_readiness.v1` | runtime/readiness/action evidence freshness | compiled step count; `--query task-script-step-readiness` | no | no | no |
