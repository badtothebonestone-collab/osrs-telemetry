...
| task_run_readiness | read-only script lifecycle gate | Knowledge Fabric and `task_script_api.py` | Codex before selecting/requesting the next high-level primitive | `task_run_readiness.v1` | runtime/readiness/action/navigation evidence freshness | compiled step count, inferred primitive | no | no | no |
