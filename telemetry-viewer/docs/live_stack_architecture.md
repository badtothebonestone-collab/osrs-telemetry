...
4a. High-level task script API
   ...
   - Run-readiness queries infer the next high-level primitive from current lifecycle evidence, then nest the step-readiness gate and action/input visibility proof. They label mixed/stale lifecycle context as advisory-only when loaded scene proof is missing or unverified. They do not execute the request.
