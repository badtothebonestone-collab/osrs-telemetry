...
def build_from_daemon(status: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        ...
        "selectedServiceTargetName": service_context.get("selectedServiceTargetName", status.get("selectedServiceTargetName")),
        "selectedServiceTargetTile": service_context.get("selectedServiceTargetTile", status.get("selectedServiceTargetTile")),
        "distanceToServiceTarget": service_context.get("distanceToServiceTarget", status.get("distanceToServiceTarget")),
        "arrivedAtFinalApproach": service_context.get("arrivedAtFinalApproach", status.get("serviceArrivedAtFinalApproach")),
        "arrivedNearDestination": service_context.get("arrivedNearDestination", status.get("serviceArrivedNearDestination")),
        "distanceToFinalApproach": service_context.get("distanceToFinalApproach", status.get("serviceDistanceToFinalApproach")),
...
def format_human(payload: dict[str, Any]) -> str:
    lines = [
        ...
        f"Service ready: {'yes' if payload.get('serviceReady') else 'no'}",
        ...
        f"Selected service target: {payload.get('selectedServiceTargetName') or 'none'}",
        f"Selected service target tile: {payload.get('selectedServiceTargetTile') or 'none'}",
        f"Distance to service target: {payload.get('distanceToServiceTarget') if payload.get('distanceToServiceTarget') is not None else 'unknown'}",
        f"Arrived at final approach: {'yes' if payload.get('arrivedAtFinalApproach') else 'no'}",
        f"Arrived near destination: {'yes' if payload.get('arrivedNearDestination') else 'no'}",
