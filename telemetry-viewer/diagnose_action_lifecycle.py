...
def format_human(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"  waiting for: {payload.get('expectedSignal') or lifecycle.get('expectedSignal') or expected.get('expectedSignal') or 'unknown'}",
        f"  observed signals: {', '.join(str(item) for item in (payload.get('observedSignals') or lifecycle.get('observedSignals') or observed.get('observedSignals') or [])) or 'none'}",
        f"  result complete: {payload.get('resultComplete') if payload.get('resultComplete') is not None else lifecycle.get('resultComplete')}",
        f"  result outcome: {payload.get('resultOutcome') or lifecycle.get('resultOutcome') or observed.get('resultOutcome') or 'unknown'}",
        f"  elapsed: {payload.get('elapsedTicks') if payload.get('elapsedTicks') is not None else lifecycle.get('elapsedTicks')} ticks / {payload.get('elapsedMillis') if payload.get('elapsedMillis') is not None else lifecycle.get('elapsedMillis')} ms",
        f"  timeout: {payload.get('timeoutTicks') if payload.get('timeoutTicks') is not None else lifecycle.get('timeoutTicks')} ticks / {payload.get('timeoutMillis') if payload.get('timeoutMillis') is not None else lifecycle.get('timeoutMillis')} ms",
        f"  next action allowed: {payload.get('nextActionAllowed') if payload.get('nextActionAllowed') is not None else lifecycle.get('nextActionAllowed')}",
        f"  wait started: {payload.get('waitStartedTick') or lifecycle.get('waitStartedTick') or 'unknown'}",
