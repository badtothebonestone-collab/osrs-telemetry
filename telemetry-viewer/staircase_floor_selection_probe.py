...
SCHEMA = "staircase_floor_selection_probe.v1"
ATTEMPT_SCHEMA = "staircase_floor_selection_probe_attempt.v1"
...
DEFAULT_TOP_FLOOR_STAIRCASE = {"worldX": 3205, "worldY": 3229, "plane": 2}
DEFAULT_TOP_FLOOR_PLAYER = {"worldX": 3209, "worldY": 3220, "plane": 2}
DEFAULT_TOP_FLOOR_STAIRCASE_ID = 56231
...
def collect_staircases(status: dict[str, Any], player: dict[str, int] | None, *, radius_tiles: int) -> list[dict[str, Any]]:
    ...
    objects.sort(
        key=lambda item: (
            0
            if item.get("objectId") == DEFAULT_TOP_FLOOR_STAIRCASE_ID and _tile(item.get("world")) == DEFAULT_TOP_FLOOR_STAIRCASE
            else 1,
...
def _schema_gap(attempts: list[dict[str, Any]]) -> str:
    ...
    lines = ["# Staircase Floor-Selection Probe Schema Gap Report", ""]
    if stale_menu:
    ...
    lines.append("")
    lines.append("- The probe does not perform a route transition; route-guide enrichment is limited to menu availability evidence.")
    return "\n".join(lines) + "\n"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Focused top-floor Staircase floor-selection hover/menu evidence probe.")
    ...
    parser.add_argument("--target-world-x", type=int, default=DEFAULT_TOP_FLOOR_STAIRCASE["worldX"])
    parser.add_argument("--target-world-y", type=int, default=DEFAULT_TOP_FLOOR_STAIRCASE["worldY"])
    parser.add_argument("--target-plane", type=int, default=DEFAULT_TOP_FLOOR_STAIRCASE["plane"])
    parser.add_argument("--target-object-id", type=int, default=DEFAULT_TOP_FLOOR_STAIRCASE_ID)
    parser.add_argument("--player-world-x", type=int, default=DEFAULT_TOP_FLOOR_PLAYER["worldX"])
    parser.add_argument("--player-world-y", type=int, default=DEFAULT_TOP_FLOOR_PLAYER["worldY"])
    parser.add_argument("--player-plane", type=int, default=DEFAULT_TOP_FLOOR_PLAYER["plane"])
    parser.add_argument("--near-radius-tiles", type=int, default=14)
...
def run_probe(args: argparse.Namespace) -> dict[str, Any]:
    ...
    out_dir = Path(args.out_dir) if args.out_dir else Path("recordings") / f"{stamp}_staircase_floor_selection_probe"
    summary_path = out_dir / "staircase_floor_selection_probe.json"
    attempts_path = out_dir / "staircase_floor_selection_probe.jsonl"
    ...
    near_top_floor_state = bool(player and player["plane"] == args.player_plane and near_distance is not None and near_distance <= args.near_radius_tiles)
    ...
    exact_staircases = [
        item
        for item in staircases
        if item.get("objectId") == args.target_object_id and _tile(item.get("world")) == target_world
    ]
    blockers: list[str] = []
    warnings: list[str] = []
    if not near_top_floor_state:
        blockers.append("player_not_near_top_floor_route_state")
    if input_geometry.get("status") != "PASS":
        blockers.append("input_geometry_unavailable")
    if not staircases:
        blockers.append("top_floor_staircase_objects_missing")
    if staircases and not exact_staircases:
        blockers.append("expected_top_floor_staircase_missing")
    selected_staircases = (exact_staircases or staircases)[: max(1, int(args.max_targets or 1))]
    payload: dict[str, Any] = {
        ...
        "nearTopFloorRouteState": near_top_floor_state,
        "nearDistanceTiles": near_distance,
        "targetWorld": target_world,
        "targetObjectId": args.target_object_id,
        "currentWorld": player,
        "currentPlane": player.get("plane") if player else None,
        ...
        "staircaseObjectId": selected_staircases[0].get("objectId") if selected_staircases else None,
        "staircaseWorld": selected_staircases[0].get("world") if selected_staircases else None,
        "staircasePlane": _dict(selected_staircases[0].get("world")).get("plane") if selected_staircases else None,
        "attempts": [],
        "capturedOptions": [],
        "matchingMenuEntries": [],
        "menuRows": [],
        ...
        "middleFloorAvailable": False,
        "topFloorAvailable": False,
        "climbDownAvailable": False,
        "climbUpAvailable": False,
            ...
            for point_index, canvas_point in enumerate(points):
                ...
                plan = plan_mouse_movement(
                    ...
                    MouseTarget(
                        ...
                        label="staircase_floor_selection_probe",
    ...
    first_menu = next((_dict(attempt.get("rightClickMenu")) for attempt in attempts if _list(_dict(attempt.get("rightClickMenu")).get("entries"))), {})
    if not first_menu:
        first_menu = next((_dict(attempt.get("hoverMenu")) for attempt in attempts if _list(_dict(attempt.get("hoverMenu")).get("entries"))), {})
    payload["menuRows"] = _list(first_menu.get("entries"))
    payload["rowBounds"] = [_dict(entry).get("rowBounds") for entry in payload["menuRows"] if _dict(entry).get("rowBounds")]
    payload["topMenuOption"] = first_menu.get("topOption")
    payload["topMenuTarget"] = first_menu.get("topTarget")
    payload["bottomFloorAvailable"] = any(_norm(entry.get("option")) == "bottom floor" for entry in matching_entries)
    payload["middleFloorAvailable"] = any(_norm(entry.get("option")) == "middle floor" for entry in matching_entries)
    payload["topFloorAvailable"] = any(_norm(entry.get("option")) == "top floor" for entry in matching_entries)
    payload["climbDownAvailable"] = any(_norm(entry.get("option")) == "climb down" for entry in matching_entries)
    payload["climbUpAvailable"] = any(_norm(entry.get("option")) == "climb up" for entry in matching_entries)
    payload["menuRowBoundsCaptured"] = any(_dict(entry).get("rowBounds") for entry in matching_entries)
    payload["evidenceFreshness"] = {
        "status": "FAIL" if menu_evidence_stale else "PASS" if matching_entries else "UNKNOWN",
        "menuEvidenceStale": menu_evidence_stale,
        "staleCapturedOptions": payload["staleCapturedOptions"],
    }
    payload["status"] = "PASS" if payload["bottomFloorAvailable"] else "WARN"
    if menu_evidence_stale:
        payload["blockers"].append("plugin_menu_evidence_stale")
    if not payload["bottomFloorAvailable"]:
        payload["blockers"].append("bottom_floor_option_not_captured")
    _write_json(summary_path, payload)
